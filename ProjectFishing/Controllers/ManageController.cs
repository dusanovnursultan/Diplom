...
namespace ProjectFishing.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> ChangeImage(HttpPostedFileBase files)
        {
            if (!ModelState.IsValid || files == null && files.ContentLength == 0) //проверяем не пустое ли значение
                return RedirectToAction("Index");
            string pathToSave = Server.MapPath("~/Images/ForViews/Manage"); // берем путь куда будем сохранять
            string ext = System.IO.Path.GetExtension(files.FileName); // берем расширение для файла
            var id = Guid.NewGuid();
            string uniqueName = $"{"ImageUser"}{id}{files.FileName}{ext}"; // создаем уникальное имя для файла
